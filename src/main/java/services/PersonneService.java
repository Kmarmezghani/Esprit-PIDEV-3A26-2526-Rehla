...
public class PersonneService implements IService<Personne> {
    ...
    public void add(Personne p) {
        ...
        // Table `personne` in DB has NOT NULL heureNotif and a notifSmsActive flag.
        // We insert sane defaults so INSERT never fails on strict SQL modes.
        String sql = "INSERT INTO `personne` " +
                "(`nom`, `prenom`, `email`, `motDePasse`, `dateInscription`, `role`, `statutCompte`, `telephone`, `heureNotif`, `notifSmsActive`) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            java.time.LocalTime notifTime =
                    (p.getHeureNotif() != null ? p.getHeureNotif() : java.time.LocalTime.MIDNIGHT);
            ps.setTime(9, Time.valueOf(notifTime));

            Boolean smsActive = (p.getNotifSmsActive() != null ? p.getNotifSmsActive() : Boolean.TRUE);
            ps.setBoolean(10, smsActive);
    ...
    public void update(Personne p) {
        if (conn == null) throw new IllegalStateException("DB not connected");
        String sql = "UPDATE `personne` SET `nom`=?, `prenom`=?, `email`=?, `motDePasse`=?, `role`=?, `statutCompte`=?, `telephone`=?, `heureNotif`=?, `notifSmsActive`=?, `profile_photo`=? WHERE `id`=?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            ps.setTime(8, p.getHeureNotif() != null ? Time.valueOf(p.getHeureNotif()) : Time.valueOf(java.time.LocalTime.MIDNIGHT));
            ps.setBoolean(9, p.getNotifSmsActive() != null ? p.getNotifSmsActive() : true);
