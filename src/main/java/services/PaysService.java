...
public class PaysService implements IService<Pays> {
    ...
    public void add(Pays pays) {
        ...
        String SQL = "INSERT INTO pays (nom, description, visit_count) VALUES (?, ?, ?)";
        ...
        try {
            ...
            pstmt.setInt(3, pays.getVisitCount());
    ...
    public void update(Pays pays) {
        ...
        String SQL = "UPDATE pays SET nom = ?, description = ?, visit_count = ? WHERE id = ?";
    ...
    public List<Pays> getAll() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                pays.setVisitCount(rs.getInt("visit_count"));

                pays.setImage(rs.getString("image"));
    ...
    public Pays getById(int id) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                System.out.println("DEBUG: Database pays.id=" + id + " image='" + imageValue + "'");

                pays.setImage(imageValue);
    ...
    public void incrementVisitCount(int paysId) {
        if (conn == null) return;
        String SQL = "UPDATE pays SET visit_count = visit_count + 1 WHERE id = ?";
        try {
            PreparedStatement pstmt = conn.prepareStatement(SQL);
            pstmt.setInt(1, paysId);
            pstmt.executeUpdate();
            System.out.println("Pays visit count incremented successfully!");
        } catch (SQLException e) {
            System.out.println(e.getMessage());
        }
    }
