...
void goToMyReservations(ActionEvent event) {
    ...
    private void initProfileInfo() {
        ...
        try {
            if (currentUser != null && currentUser.getProfilePhoto() != null && !currentUser.getProfilePhoto().isEmpty()) {
                File photoFile = new File(currentUser.getProfilePhoto());
                if (photoFile.exists()) {
                    String uri = photoFile.toURI().toString();
                    avatarImage = new Image(uri, 120, 120, true, true);
                    smallAvatar = new Image(uri, 36, 36, true, true);
                } else {
                    avatarImage = new Image(
                            getClass().getResource("/Backoffice/icons/usericon.png").toExternalForm(),
                            120, 120, true, true
                    );
                    smallAvatar = new Image(
                            getClass().getResource("/Backoffice/icons/usericon.png").toExternalForm(),
                            36, 36, true, true
                    );
                }
